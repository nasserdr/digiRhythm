...
\author{Hassan-Roland Nasser~\orcidlink{0000-0003-1821-3234}\\Agroscope - Digital Production
   \And Joanna Stachowicz~\orcidlink{0000-0003-4921-8353}\\Agroscope - Digital Production
   \AND Marie Schneider~\orcidlink{0000-0003-4921-8353}\\Johann Heinrich von Th\"unen\\ Institut für \"Okologischen Landbau
   \And Christina Umst\"atter~\orcidlink{0000-0002-7638-3867}\\Johann Heinrich von Th\"unen\\ Institut für Agrartechnologie}
\Plainauthor{Hassan-Roland Nasser, Joanna Stachowicz, Marie Schneider, Christina Umst\"atter}
...
\Abstract{
Animals' welfare is becoming increasingly attractive, especially in livestock farming. One method to quantify an animal's welfare is measuring its rhythmicity from activities acquired with sensors (e.g., accelerometers) mounted on the animal. The Degree of functional coupling (DFC) is one of these rhythmicity indicators. Computing the DFC involves several computational steps, such as computing a frequency spectrum and determining the statistical significance of its peaks. Despite the importance of this indicator and its use in the literature, no open source library is available for computing it. In this paper, we present the \proglang{R} package \pkg{digiRhythm}, which provides off-the-shelves functionalities for measuring animal's rhythmicity, and other valuable add-ons such as creating actrograms, computing and visualizing average activities, and the calculation of the diurnality index. We gave special attention to integrating the package in the research workflow, from raw data to creating publication-ready content.
}
...
\Address{
  Hassan-Roland Nasser, Joanna Stachowicz\\
  ...
  Christina Umst\"atter\\
  Johann Heinrich von Th\"unen Institut f\"ur Agrartechnologie \\
  Bundesallee 47 \\
  38116 Braunschweig\\
  Deutschland\\
  E-mail: \email{christina.umstaetter@thuenen.de}\\
  URL: \url{https://www.thuenen.de/de/fachinstitute/agrartechnologie/personal/leitung/prof-dr-christina-umstaetter}\\
...
\section[Introduction: Degree of functional coupling and animal welfare ]{Introduction: Degree of functional coupling and animal welfare} \label{sec:intro}

The Degree of functional coupling (DFC) is a metric used to describe the extent to which a measured activity is significantly cyclic to 24 hours and thus harmonically synchronized with the periodicity of the environment. The activity can be, e.g., the number of steps during a time interval or an acceleration measured with an accelerometer mounted on the animal. The DFC was first introduced by \cite{sinz1976systemanalyse} to gain insight into the circadian rhythm of sheep and used by \cite{scheibe1999comparative} to zoo animals and wildlife by applying a sequence of mathematical operations on continuous signals collected via animal-borne sensors. With the increasing availability of sensor systems to collect behavioral data continuously, more studies using the DFC were undertaken \citep{sarout2018assessment, fuchs2022detecting}. Throughout multiple studies (some refs), it has been proven that the DFC can be used as a meaningful indicator to assess an animal's welfare, thus the importance of having an open source package specifically for this type of computation. Therefore, an \proglang{R} package was developed and published with name \pkg{digiRhythm} \citep{dgm}.
...
This paper is organized as follows. In section \ref{sec:dfc}, the mathematical details of computing the DFC are explained. A deep dive into the statistical significance of frequency peaks needed for the DFC computation is also highlighted. In section \ref{sec:usage}, a detailed overview on all the \pkg{digiRhythm} package functionalities is provided. Finally, conclusions are drawn in section \ref{sec:summary}.

\section{Understanding the degree of functional coupling}{\label{sec:dfc}}

Computing the DFC involves several steps. First, the Lomb-Scargle Periodogram \citep{lomb1976least, scargle1982studies, vanderplas2018understanding} of the activityis computed. The second step investigates the spectrum by looking at specific frequencies (harmonics and non-harmonics, further details in Figure \ref{fig:reallsp}). Afterward, the statistical significance of frequency peaks is computed. Finally, a ratio of the sum of frequency powers is calculated (a detailed breakdown of the calculation is given in Figure \ref{dfc_sum}). It was decided to follow a top down approach and start with the  explaining the the ratio of frequency powers. Afterwards, we deep dive into how these frequency powers and their statistical significance can be computed in details.
...
The activityis in the time domain (Figure \ref{fig:signalVslsp}, left) and can be transformed to the frequency domain (Figure \ref{fig:signalVslsp}, right) using the LSP. In the time domain, we can see the magnitude of the activity at a particular time stamp. In the frequency domain we can see the magnitude (power) of oscillations of specific frequencies present in the activity. For example, a  sine wave of 24 hours (Figure \ref{fig:signalVslsp}, left, first row) period could be represented with a one-peak periodogram (Figure \ref{fig:signalVslsp}, right, first row); this peak represents the frequency of a sine wave of 24 hours period. The same idea could be illustrated for a sine wave of 6 hours period (Figure \ref{fig:signalVslsp}, second row), and so on. Moreover, an activitycomposed of the addition of 24 hours and 6 hours sine waves (Figure \ref{fig:signalVslsp}, left, third row) will result in having two peaks for the periodogram (Figure \ref{fig:signalVslsp}, right, third row). Consequently, a periodogram is a tool that allows us to infer the power of each possible sine wave present in the activity.

\begin{figure}
  \includegraphics[width=1\columnwidth]{signalVslsp}
  \caption{Examplary harmonics (left) and their corresponding LSP (right). The first two rows show pefect sine waves acivity signals with 24 hours and 6 hours periods, respectively, and their corresponding LSP. The bottom row shows the LSP (right) of two sine waves (of 24 hours period and 6 hours period) added (left) together.}
  \label{fig:signalVslsp}
\end{figure}

Activity signals acquired from sensors on animals are far from perfect sine waves. Therefore, investigating periodic events inside these signals needs a conversion through LSP. Figure \ref{fig:realsignal} shows an example of a real activity example - the motion index (which is a proprietary metric of the
...
Before computing the LSP, we have to determine the frequency grid on which we will apply the formula \ref{eq:lsp}. The frequency grid goes from 0 to $f_{max}$ which is the maximum frequency whom power can be computed from the activity $g_n$. $f_{max}$ is determined according to the Shanon-Nyquist theorem (\cite{shannon1928certain, shannon1949mathematical}).

Figure \ref{fig:reallsp} represent the Lomb-Scargle periodogram computed from the activitypresent in Figure \ref{fig:realsignal}.
...
head(lsp$lsp_data)
@
\caption{The Lomb-Scargle Periodogram of the activity data show in Figure \ref{fig:realsignal}. Red bars show the amplitude of harmonic frequencies (with the corresponding harmonic period), while green bars represent non-harmonic frequencies. The dashed line represents the threshold of significance (frequencies with power less than this threshold are considered non-significant). In this particular example, there are only two frequencies with significant power; one of them - which corresponds to a 12-hours period - is harmonic.}
\label{fig:reallsp}
\end{figure}

\subsection{Harmonic frequencies and the intuition behind}{\label{sub:harmonic}}
Animal species exhibit a circadian rhythm reflected in behavioral changes following a 24 hours cycle. Therefore, rhythmic behavioral events are reflected in the power of harmonic frequencies of the primary cycle (24 hours). These harmonics are the frequencies that correspond to 24 hours period divided by any integer smaller than 24 (24/1, 24/2, …). At this point, it is important to recall the subset of frequency powers previously described. These subsets are illustrated in Figure \ref{dfc_sum} for clarification.

\begin{figure}[t]
  \includegraphics[width=1\columnwidth]{sum_dfc}
  \caption{An illustration shows the frequency peaks found in a Lomb Scargle Periodogram in the context of animals' rhythmicity. The whole frequency space consists of frequencies with significant and non-significant peaks, whose powers sum up to $sumall$. The powers of the frequencies with significant peaks sum up to $sumsig$. A subset of the significant peaks represents harmonic frequencies (24h, 24h/2, 24h/3, \dots) whose powers sum up to $ssh$.}
  \label{dfc_sum}
\end{figure}

The relationship between rhythmicity and frequencies would now be clear for the reader. If the significant frequencies are only the harmonic ones (i.e., ssh = sumsig), then the DFC = 1, and therefore the animal exhibits a highly rhythmic behavior. On the counterpart, if the significant frequencies are not harmonic, then the DFC < 1; therefore, the animal exhibits a low rhythmic behavior.

\subsection{Measuring the statistical significance of a frequency peak}{\label{sub:significance}}
After computing the periodogram, measuring the statistical significance of  frequency peaks is the second step in the DFC process. The latter would show a peak per frequency. Determining whether a certain peak reflects a activitycomponent or a noise component is a crucial step. In fact, for a small number of samples and a low signal-to-noise ratio, the noise peaks become comparable to the activity peaks. Hence, the importance of having a tool that measures the statistical significance of a peak.
In this regard, the False Alarm Probability (FAP) is the measure that is typically used. The FAP reflects the probability that data with no activitywould lead to a peak of similar magnitude.

According to \cite{scargle1982studies}, the values of the un-normalized periodogram follow a $\chi^2$ distribution with two degrees of freedom. This implies that the probability of observing periodogram values less than a given $Z=P(f_0)$ in an activityformed by only Gaussian noise is:
%
\begin{equation}
  P_{single} = 1 - \exp{Z}
\end{equation}
%
To compute the FAP, there are multiple approaches. According to \citep{suveges2015comparative}, the Baluev method \citep{baluev2008assessing} is one of the top performing ones. This method was chosen to be implemented in \pkg{digiRhythm}.
...
In addition to the ability to compute the DFC and HP, \pkg{digiRhythm} contains much more functionalities. These functionalities were complied with animal behavior scientists to provide the commonly needed tools for a rhythmicity analysis topic. Currently, the library offer the following tools:
...
The \fct{dfc} function is a wrapper of the  that users could also use should they wish to investigate more about
\subsection{The input data format}
As mentioned in section \ref{sec:intro}, the library accepts input Data frame s with a specific structure. A typical data frame sampled at 15 min intervals is shown below. This data frame is provided with the \pkg{digiRhythm} package and could be fetched using:
...
\begin{figure}[t!]
\centering
<<print_dfc, fig=TRUE, cache = TRUE, width=9>>==
library("ggplot2")
plot(my_dfc)
@
...
It is possible to access the computed data and how the user can change the plot based on the returned plot.
Calling the data field from the variable \code{my_dfc} will allow accessing the computed DFC and HP as shown below:

It is, therefore, easy to access the resulting DFC and HP and therefore use them in further analysis like statistical modeling or more customized workflows. Because the returned object has the \pkg{ggplot2} type \citep{wickham2010layered}, it is also possible to change the plot directly by adding a layer on the aesthetics of the \code{my_dfc} object as shown below:
...
@
%
\caption{\label{fig:dfc_modified} The output plot from Figure \ref{fig:dfc} modified by adding a layer of aesthetics causing some modifications to the \fct{ggplot} output returned by the \fct{dfc}. In particular, the font family was changed to \emph{Times}, the font size to 8 points, and the y-axis text was changed to a bigger size (15) and colored red.}
\end{figure}


This is an illustrative yet straightforward example to sketch the feature of resuing the graphical outputs of the library, but the application comes in handy when users need to produce publication-quality pictures where they should abide by a specific format. It is important to mention that this feature is possible because the functions were designed in a way they return \fct{ggplot} objects rather than base \proglang{R} objects.
...
The function \fct{daily\_average\_activity} compute and plot this indicator as shown in the Figure \ref{fig:avg_act}.
...
The Diurnality index is also an important measure when it comes to assessing animals' rhythmicity. This measure indicates whether the animal exercises its activity more during the daytime or the nighttime. Using the Diurnality function, the user has the liberty to choose the day and light time. The user can also omit some periods during the day, such as milking time, where the milking process controls the animal's activity rather than its behavior. The Diurnality index is computed according to \cite{hoogenboom1984seasonal}:
...
The \pkg{digiRhythm} package can make a significant contribution to the animal welfare assessment as it makes the analysis of animal rhythmicity more accessible. The tools is easy to uses and comes in handy especially it is delivered as an \proglang{R} package. The theoretical details and intuition behind the DFC were explained in section \ref{sec:dfc}. The usage of the package was explained in the section \ref{sec:usage}. Further usage examples could be found in the library's vignettes.

At the date of writing this paper, the current version (v1.1) of the library does not offer the possibility to compute rhythmicity with other techniques such as cosinor analysis (\cite{refinetti2016}). Therefore, there is potential to expand the library's functionality in the future to include other techniques.

Furthermore, it would be interesting to investigate the correlation between the DFC and rhythmicity computed on data from various sources or techniques. In our study, we obtained rhythmicity measures from sensors on animals. It would be valuable to examine the correlation between the rhythmicity of these measures and those obtained from other sources within the same animals, such as blood samples. For instance, \cite{moller2022diurnal} utilized a range of techniques, including set logic, circular cross-correlation, circular clustering, functional enrichment analyses, and least squares regression, on transcriptomic time series data from humans and baboons. Additionally, \cite{plaza2022gps} used GPS data to estimate rhythmicity. Comparing the correlations between rhythmicity measures derived from different types of data and between various techniques could be a useful research area.


Finally, We invite other researchers to contribute to this library by submitting data, code, and techniques that can be shared with the scientific community. By collaborating and sharing these techniques, we can collectively advance our understanding of rhythmicity and its role in various biological systems.
