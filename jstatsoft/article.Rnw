...
\item \fct{sliding\_DI}: With this function, users can compute and graphically present the diurnality index, using specific definitions of day/night per day.
...
In the above example for the \fct{diurnality} function, the day starts at 6:30 AM and ends at 4:30 PM, while the night starts at 6 PM and ends at 5 AM. Note that the \fct{sliding\_DI} function would compute the DI for each day in the dataset based on the specified day/night periods, of that day. This could provide a more accurate reflection of the individual's behavior, as it accounts for changes daily routines, that externally control the organisms activity or changes in daylight hours throughout the year.
