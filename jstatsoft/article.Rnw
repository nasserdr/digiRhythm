...
The function \fct{sliding\_DI} computes the diurnality index as described above. However, instead of using a single definition of day and night for the complete computation period, a data frame defining different definitions of the start/end of day/night is included. The data frame must include five columns, one date column, and four columns of timestamps for start/end of day/night. All of these columns must be formatted as POSIXct.

This function, which includes different definitions, is useful in various settings, as it allows to improve the computation accuracy. For example, in Europe, it could be used to reduce the bias emerging by converting summertime and wintertime to a time zone, such as GMT, or by defining sunrise and sunset.
