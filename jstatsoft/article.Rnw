...
The function \fct{sliding\_DI} also computes the diurnality index as described above. The difference is that no single definition of day and night was used for the complete calculation period. It is possible to include a data frame with different definitions of the start/end of day/night (namely, the variable $day\_start$, $day\_end$, and $night\_start$ and $night\_end$). The data frame should include five columns, one date column, and four columns of timestamps for start/end of day/night. All of these columns should be in the POSIXct format.

This method, which includes different definitions, could be helpful in various experiments. For example, in Europe, it could be used to reduce the bias emerging by converting summertime and wintertime to a time zone, such as GMT. An additional use could be including sunrise and sunset by using the same definition for $day\_start$, $day\_end$, and $night\_start$ and $night\_end$.
